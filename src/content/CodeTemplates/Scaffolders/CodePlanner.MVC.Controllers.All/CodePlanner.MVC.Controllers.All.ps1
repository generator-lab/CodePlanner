[T4Scaffolding.Scaffolder(Description = "Enter a description of CodePlanner.MVC.Controllers.All here")][CmdletBinding()]
param(        
    [string]$Project,
	[string]$CodeLanguage,
...
$classes | ForEach{		
	...
	$_.Bases | ForEach{
		if($_.Name -eq "PersistentEntity"){							
			Scaffold CodePlanner.MVC.Controller.For $current.Name -Force:$Force			
		}
		if($_.Name -eq "PersistentTrackingEntity"){							
			Scaffold CodePlanner.ScaffoldBackend.For $current.Name -Force:$Force			
		}
	}		
}